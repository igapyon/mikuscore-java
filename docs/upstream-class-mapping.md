...
| `src/ts/abc-io.ts` | pending | not started | ABC I/O |
| `src/ts/abc-lexer.ts` | pending | not started | ABC lexer |
| `src/ts/abc-parser.ts` | pending | not started | ABC parser |
...
| `src/ts/cli-api.ts` | `jp.igapyon.mikuscore.coreapi.CoreApi`, `jp.igapyon.mikuscore.musicxml.MusicXmlState` | partial | `summarizeMusicXmlState`, `inspectMusicXmlMeasure`, `validateMusicXmlCommand`, `applyMusicXmlCommand`, and `diffMusicXmlState` migrated first |
| `scripts/mikuscore-cli.mjs` | `jp.igapyon.mikuscore.cli.MikuscoreCli` | partial | `--help`, `--version`, `state summarize`, `state inspect-measure`, `state validate-command`, `state apply-command`, and `state diff` |
